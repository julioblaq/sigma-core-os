...
Railway managed PostgreSQL and Redis were not provisioned in this pass because `railway add --database postgres` and `railway add --database redis` returned `Unauthorized` even after CLI login. Revisit from the Railway dashboard or after account/project permissions are refreshed.

The `sigma-api` Railway volume at `/data` exists, but the current non-root container cannot open `/data/sigma.db` after the Railway volume is mounted. The live service currently uses `/tmp/sigma.db` as a temporary cloud cutover path. Fix persistence before relying on cloud state as durable production memory.
...
3. Fix `/data` Railway volume ownership for the non-root API container, or migrate to PostgreSQL before production traffic.
4. Keep SQLite on a single Railway volume only as a temporary bridge, or migrate to PostgreSQL before multi-replica usage.
