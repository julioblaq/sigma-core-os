...
//
// Uses randomized suffixes on all workspace names to prevent SLUG_TAKEN errors
...
describe('createWorkspace', () => {
  it('creates a workspace with correct name and slug', () => {
    const name = uid('Sigma Futures Alpha');
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
    const { workspace } = createWorkspace(name, 'user-founder');
    assert.ok(workspace.id, 'should have an id');
    assert.equal(workspace.name, name);
    assert.equal(workspace.slug, slug);
  ...
  it('creator automatically becomes admin', () => {
    const { workspace, member } = createWorkspace(uid('Creator Admin Test'), 'user-creator');
  ...
  it('unique slug enforcement: duplicate name throws SLUG_TAKEN', () => {
    const name = uid('Unique Slug Workspace');
    createWorkspace(name, 'user-a');
    assert.throws(
      () => createWorkspace(name, 'user-b'),
      (err: unknown) => {
  ...
  it('different name with different slug succeeds', () => {
    const { workspace: ws1 } = createWorkspace(uid('Prop Firm Team'), 'user-pt1');
    const { workspace: ws2 } = createWorkspace(uid('Prop Firm Group'), 'user-pt2');
...
describe('addMember', () => {
  it('adds a member with specified role', () => {
    const { workspace } = createWorkspace(uid('Member Test Workspace'), 'user-admin1');
  ...
  it('admin can manage members (canManageMembers)', () => {
    const { workspace } = createWorkspace(uid('Admin Manages Workspace'), 'user-admin2');
  ...
  it('invalid role rejected with INVALID_ROLE', () => {
    const { workspace } = createWorkspace(uid('Invalid Role Test'), 'user-admin3');
    assert.throws(
  ...
  it('setMemberRole changes role correctly', () => {
    const { workspace } = createWorkspace(uid('Role Change Workspace'), 'user-admin4');
...
describe('role permissions', () => {
  ...
  it('viewer cannot approve a real approval (approval spine integration)', () => {
    const { workspace } = createWorkspace(uid('Approval Role Workspace'), 'user-admin5');
  ...
  it('approver can approve a real approval (approval spine integration)', () => {
    const { workspace } = createWorkspace(uid('Approver Workspace'), 'user-admin6');
