 ...
 * Run: node --experimental-vm-modules --import tsx/esm tests/approval-spine.test.ts
 * Or:  npx tsx --test tests/approval-spine.test.ts
 */
...
// Use an isolated in-memory DB for tests by setting env before importing db
process.env['SIGMA_DB'] = ':memory:';
...
describe('Approval spine', () => {
           ...
           describe('deny flow', () => {
                 it('resolveApproval sets status to denied with reason', () => {
                        ...
                        it('denial without reason returns 400 from API (server-level guard)', () => {
                                // This is a logic test - verify the guard condition directly
           ...
           describe('immutability - double-resolution prevention', () => {
                 it('cannot approve an already-approved record', () => {
                         ...
                         // Second resolution must return null
                        ...
                        it('status is immutable after resolution', () => {
                                ...
                                // Even after an attempted second resolution, status must stay denied
